...
export function buildRightNow(congestion: string | null): string {
  ...
  if (level === "약간 붐빔") {
    ...
    return `It's a bit busy right now (${timeCtx}). ${avoidTip}`;
  }
  if (level === "붐빔") {
    return `It's quite crowded right now (${timeCtx}). Visiting in 1–2 hours or early morning is recommended.`;
  }
  if (level === "매우 붐빔") {
    return `It's very crowded right now (${timeCtx}). If possible, visit at a different time.`;
  }
...
export function buildPrompt(
  ...
  const ctx: string[] = [`Now: ${now} (${weekday}, ${period})`];
  if (operating_time) ctx.push(`Hours: ${operating_time}`);
  if (fee) ctx.push(`Fee: ${fee}`);
  if (subway) ctx.push(`Transit: ${subway}`);
  if (congestion) {
    const level = congestion.split(":")[0]?.trim();
    const levelEn = level ? CONGESTION_EN[level] ?? level : "";
    ctx.push(`Live crowd level: ${levelEn}${levelEn ? ` — ` : ""}${congestion} (raw data in Korean)`);
  }
